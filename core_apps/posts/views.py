...
class UnBookmarkPostAPIView(APIView):
    def patch(self, request, slug):
        ...
        if user not in post.bookmarked_by.all():
            return Response({"message":"You can't remove a bookmar that did not exist in the first place"}, status= status.HTTP_400_BAD_REQUEST)
