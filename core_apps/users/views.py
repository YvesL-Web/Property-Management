...
def set_auth_cookies(response:Response, access_token: str, refresh_token:Optional[str]=None) -> None:
    ...
    if refresh_token:
        refresh_token_lifetine = settings.SIMPLE_JWT["REFRESH_TOKEN_LIFETIME"].total_seconds()
    ...
    def post(self, request: Request, *args, **kwargs) -> Response:
        ...
        if provider_res.status_code == status.HTTP_201_CREATED:
            ...
            if access_token and refresh_token:
                ...
                provider_res.data["message"] = "Access or refresh not found in provider response."
                logger.error(
